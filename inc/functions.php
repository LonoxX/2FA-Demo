...
class UserClass
{
    public function userLogin($usernameOrEmail, $password, $secret)
    {
        try {
            $db = getDB();
            $hash_password = hash('sha256', $password);
            $stmt = $db->prepare("SELECT uid FROM users WHERE (username=:usernameOrEmail OR email=:usernameOrEmail) AND password=:hash_password");
            $stmt->execute([
    ...
    public function userDetails($uid)
    {
        try {
            ...
            throw new Exception('User details retrieval failed: ' . $e->getMessage());
        }
    }
}
